...
- **NEVER implement screen shake by randomizing `position` (or `randf` on `offset` as the whole system)** — Use a dedicated Trauma/Noise system layered on follow ([camera_shake_trauma_pro.gd](../scripts/camera_systems_camera_shake_trauma_pro.gd)).
- **NEVER parent the Camera directly to a high-speed physics body as the default rig** — Physics stutter or parent rotation causes motion sickness. Prefer `RemoteTransform2D/3D` / phantom decoupling with rotation sync disabled ([remote_transform_decoupling.gd](../scripts/camera_systems_remote_transform_decoupling.gd), [phantom_decoupling.gd](../scripts/camera_systems_phantom_decoupling.gd)).
...
| Rig | When | Script |
|-----|------|--------|
| **Default:** RemoteTransform / phantom | Player is CharacterBody / high-speed / rotates | [remote_transform_decoupling.gd](../scripts/camera_systems_remote_transform_decoupling.gd), [phantom_decoupling.gd](../scripts/camera_systems_phantom_decoupling.gd) |
...
| SpringArm3D + Camera3D | Third-person occlusion | [spring_lerp_camera_3d.gd](../scripts/camera_systems_spring_lerp_camera_3d.gd) — mask required |

## Available Scripts

> **MANDATORY**: Read before implementing the matching behavior. No `randf` shake samples in project code.

- [camera_shake_trauma_pro.gd](../scripts/camera_systems_camera_shake_trauma_pro.gd) — **MANDATORY** for any screen shake / impact juice.
- [camera_shake_trauma.gd](../scripts/camera_systems_camera_shake_trauma.gd) — Lighter trauma variant.
- [remote_transform_decoupling.gd](../scripts/camera_systems_remote_transform_decoupling.gd) — **MANDATORY** default for physics-body follow.
- [phantom_decoupling.gd](../scripts/camera_systems_phantom_decoupling.gd) — Alternate stable follow phantom.
- [spring_lerp_camera_3d.gd](../scripts/camera_systems_spring_lerp_camera_3d.gd) — **MANDATORY** before custom 3D follow springs.
- [deadzone_drag_margins.gd](../scripts/camera_systems_deadzone_drag_margins.gd) — Platformer drag/deadzone.
- [camera_follow_2d.gd](../scripts/camera_systems_camera_follow_2d.gd) — Smooth 2D follow helpers.
- [zoom_damping_controller.gd](../scripts/camera_systems_zoom_damping_controller.gd) — Non-linear zoom.
- [camera_state_machine.gd](../scripts/camera_systems_camera_state_machine.gd) — Follow / Static / Cinematic transitions.
- [cinematic_framing_logic.gd](../scripts/camera_systems_cinematic_framing_logic.gd) — Rule of thirds / lead room.
- [minimap_viewport_manager.gd](../scripts/camera_systems_minimap_viewport_manager.gd) — SubViewport update modes.
- [split_screen_setup.gd](../scripts/camera_systems_split_screen_setup.gd) — Local multi camera viewports.
- [first_person_sway.gd](../scripts/camera_systems_first_person_sway.gd) — FPS bob/sway on offset.
- [juice_camera.gd](../scripts/camera_systems_juice_camera.gd) — Combined juice helpers.
- [framing_box_camera_2d.gd](../scripts/camera_systems_framing_box_camera_2d.gd) — Multi-target AABB framing + zoom fit.
- [occlusion_aware_camera_3d.gd](../scripts/camera_systems_occlusion_aware_camera_3d.gd) — Raycast occlusion when SpringArm is insufficient.
- [trauma_debugger.gd](../scripts/camera_systems_trauma_debugger.gd) — On-screen trauma decay curve (debug builds).

## Expert Camera Architectures

### 1. Multi-target framing
Compute AABB of targets → lerp camera to center → zoom/distance to fit with margin. Keep juice shake on `offset` only. **MANDATORY:** [framing_box_camera_2d.gd](../scripts/camera_systems_framing_box_camera_2d.gd).

### 2. Occlusion (3D)
Prefer `SpringArm3D` with world collision mask; custom rigs use `intersect_ray` between ideal camera pos and target — [occlusion_aware_camera_3d.gd](../scripts/camera_systems_occlusion_aware_camera_3d.gd) (peer `godot-raycasting-queries`).

### 3. Trauma audit
Plot trauma decay (debug draw) while tuning [camera_shake_trauma_pro.gd](../scripts/camera_systems_camera_shake_trauma_pro.gd) — wire [trauma_debugger.gd](../scripts/camera_systems_trauma_debugger.gd) to `get_trauma()`. Never validate feel with raw `randf` offset demos.

> **MANDATORY** for multi-target framing, custom occlusion rigs, 2D/3D follow recipes, and cinematic transitions: [camera-expert-patterns.md](camera-systems-camera-expert-patterns.md). **Do NOT Load** when golden-path scripts already cover your rig.
