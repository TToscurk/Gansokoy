...
const gSample = (x, z) => GRID.sample(x, z);
{
   ...
   * repeat 是 30×30（非等向），三平面只吃一個尺度，取幾何平均。
   *
   * 低畫質不套：每像素多採樣兩次，低階機划不來，退回原本的單軸 UV。 */
  const TRI_SCALE = Math.sqrt(30 * 30) / GRID.size;
  let triOn = null;
  const syncTri = (idx) => {
    ...
    triOn = want;
    if (want) {
      applyTriplanar(MAT.field, {
        scale: TRI_SCALE, sharp: 4,
        rock: texMaps(rockTexture({ base: '#8a8172' }), [0.8, 0.98]),
        slope: [0.34, 0.60],
      });
    } else {
      MAT.field.onBeforeCompile = () => {};
      MAT.field.customProgramCacheKey = () => 'tri:off';
      MAT.field.map.repeat.set(30, 30);
      MAT.field.normalMap?.repeat.set(30, 30);
      MAT.field.roughnessMap?.repeat.set(30, 30);
...
core.setLUT(buildLUT(LUT_PRESETS.sunflower));

/* 環境生命：花田上方的鳥 ＋ 飄散的花瓣。 */
const amb = new Ambience(world, 67);
amb.birds({ cx: 0, cz: 0, r: 140, y: 48, count: 6, speed: 0.05 });
