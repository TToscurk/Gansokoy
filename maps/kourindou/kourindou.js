...
//
// 「路邊」要用路的實際位置算，不能寫死 z —— 原本 pz = 12.5±2.4，而主徑
// 在這一帶的中心線是 z≈8~11.5、半寬 2.5：一半的貨堆直接壓在路面上，
// 加上 junkPile 尾端那根半徑 2 公尺的擋人柱，從人間之里走進來的路整段
// 被貨堆封死（使用者截圖）。改成：問 PathNet 這個 x 的路中心在哪，
// 貨堆中心放在「路緣 + 擋人柱半徑 + 一點餘裕」之外，左右交錯。
for (let i = 0; i < 10; i++) {
  const t = i / 9;
  const px = 24 + t * 30;
  // 主徑中心線的 z（用控制點線性內插就夠 —— 這一段路本來就近乎直線）
  const pts = PATH_SEGMENTS[0].pts;
  let pz0 = pts[0][1];
  for (let k = 0; k < pts.length - 1; k++) {
    const [x0, z0] = pts[k], [x1, z1] = pts[k + 1];
    if ((px <= x0 && px >= x1) || (px >= x0 && px <= x1)) {
      pz0 = z0 + (z1 - z0) * ((px - x0) / (x1 - x0 || 1));
      break;
    }
  }
  // 2.5 路半寬 + 2.0 擋人柱 + 1.2 餘裕 = 中心至少離路心 5.7 公尺
  const side = i % 2 ? 1 : -1;
  junkPile(px, pz0 + side * (5.7 + rr(0, 1.4)), 3);
}
