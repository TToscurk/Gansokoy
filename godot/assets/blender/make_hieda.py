...
import tree_lib as TL          # 樹的骨架：跟 make_trees.py 同一份
...
class B:
    ...
    def use(self, _slot):
        """tree_lib 的材質槽切換。這支的顏色全在頂點色裡、整棟一個 mesh，
        不分槽 —— no-op。（make_trees.py 要分 bark/foliage 兩個 surface。）"""
...
# ─────────────────── 樹：遞迴分枝（不是棍子上插球）───────────────────
#
# 樹：遞迴分枝骨架 + 枝端葉團 —— 實作在 **tree_lib.py**。
#
# ⚠ 這裡以前有一份完整的私有實作，改了八版才對（舊版是「八角柱樹幹 +
# 2~3 顆球」，也就是字面意義上的棍子插球）。那八版的修正**沒有回流**到
# make_trees.py，所以散佈用的樹一直是棒棒糖，每加一種新樹就再生一次。
# 現在骨架只有 tree_lib 一份，兩邊共用：修一次，全圖都吃得到。
# 品種 profile（枝角／下垂／葉量）也在那裡 —— 差在骨架比例，不是只換 seed。
_norm3 = TL.norm3
_basis = TL.basis
_bend = TL.bend
_tube = TL.tube
_grow = TL.grow
_leaf_tuft = TL.leaf_tuft
_canopy_from_tips = TL.canopy_from_tips
_canopy_pads = TL.canopy_pads
build_tree = TL.build_tree
TREE_MAPLE, TREE_PINE = TL.TREE_MAPLE, TL.TREE_PINE
MAPLE_OLD, MAPLE_MID, MAPLE_YNG = TL.MAPLE_OLD, TL.MAPLE_MID, TL.MAPLE_YNG
