...
def _roof(bld, W, D, z_wall, pitch, overhang=0.85, thick=0.16):
    ...
    for sy in (1, -1):
        ...
        # 第一版半寬 0.030、間距 0.302 —— 肋與肋之間有 0.24m 的平坦帶，
        # 於是屋面在遠景是「亮線／暗帶」高頻交替，30m 外一條肋不到 1px，
        # 整片屋頂摩爾成一張細網（俯視構圖抓到的就是這個）。
        # 桟瓦**本來就是一體成型的波形瓦**，不是釘在平板上的木條 —— 半寬
        # 拉到 0.118 讓相鄰的桟幾乎接起來，屋面變成連續起伏。
        # 高頻交替沒了，摩爾自然沒了，而且這才是對的形。
        nk = max(12, int(2 * hw / 0.302))          # 桟瓦定尺 305mm（不動）
        for k in range(nk):
            x = -hw + (k + 0.5) * (2 * hw / nk)
        ...
        # ⚠ PHASE 1.7：列距 0.265 → 0.46。橫向也是高頻來源之一（17 列在
        # 遠景一樣不到 1px）。桟瓦的葺き足實際約 265mm，這裡放到 0.46 是
        # **可讀性優先於考證**的取捨 —— 近景仍看得出是一列一列的瓦，
        # 遠景不再是一張網。取捨寫在報告裡。
