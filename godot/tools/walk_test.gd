...
const ROUTES := {
	...
	"village": [
		...
		# 稗田邸搬到北門西側（-78,-164）之後，這條要驗的是「訪客從主入口
		# 走得到稗田邸」—— 舊終點 (-78,20) 現在是空地，測了也沒意義。
		{ "name": "北門 → 稗田邸前", "a": Vector2(0, -174), "b": Vector2(-52, -164) },
		{ "name": "廣場 → 稗田邸前", "a": Vector2(0, 30), "b": Vector2(-52, -150) },
		# 搬入真內容之後多一條「進得去」的路線：舊的佔位是實心方塊，
		# 院子根本沒有內部；現在是築地塀圍出的院落，藥醫門那個 9.6m 的
		# 缺口要真的走得進去（不然玩家只能繞著一圈牆看）。
		{ "name": "稗田邸前 → 院內", "a": Vector2(-52, -150), "b": Vector2(-72, -150) },
	],
	"trail": [
