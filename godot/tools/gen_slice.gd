...
const LOTS := [
	...
	# ⚠ 這裡是**巷口**：下一棟往東讓開 2.6m
	{"kind": "machiya_f_b", "x": 2.4, "back": 3.7, "side": 1, "yaw": -0.021,
	 "role": "house", "why": "legacy 模組。退縮最多，巷口那側的山牆才露得出來"},
	...
	# ⚠ 高的 legacy blockout（9.4m）**退到後排**，不站在街上。
	# 第一版把它擺在南側門面，結果它是一個沒有細節的白盒子、又是全場最高，
	# 整條街的第一眼全被它吃掉 —— 旁邊是有軸組有瓦的新版町家，對比太殘忍。
	# 退到 z=+15 之後它只在天際線上露出屋頂，「後面還有更深的街區」這個
	# 資訊拿到了，白盒子本身看不太到。
	{"kind": "machiya_b_a", "x": -1.5, "back": 15.4, "side": -1, "yaw": 0.06,
	 "role": "back", "why": "後排的高棟：只供天際線，不供街景"},
	# ── 轉角 ──
	{"kind": "machiya_e_a", "x": 21.6, "back": -5.6, "side": 1, "yaw": -PI * 0.5,
	 "role": "corner", "why": "轉角棟：正面朝**側街**（−x），主街只看得到它的"
		+ "山牆與側面 —— 這就是 corner condition"},
]
