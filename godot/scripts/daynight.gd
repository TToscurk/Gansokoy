...
func _apply_lamps() -> void:
	...
	for lamp in group.get_children():
...
func _lamp_group() -> Node:
	if is_instance_valid(_lamp_node):
		return _lamp_node
	# 街燈を持たない地図（trail・shrine・hieda…）では find_child が全木走査で
	# 必ず空振りする。1 地図につき 1 回だけ探す。
	if _lamp_searched or _bound_map == null:
		return null
	_lamp_searched = true
	_lamp_node = _bound_map.find_child("街燈", true, false)
	if _lamp_node != null and _lamp_node.get_child_count() > 0:
		# 行灯紙は 1 枚の共有マテリアル。1 個掴めば全灯に効く。
		var fukuro := _lamp_node.get_child(0).get_node_or_null("火袋")
