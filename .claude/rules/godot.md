...
Village facts: `docs/ningen-no-sato.md` (structure rules only — its generator
pipeline is retired). This file is the short list of traps. Generator rules
below apply to the **remaining** generators (hieda, kourindou, trail, props,
textures) — the village generator no longer exists.
...
## Scenes: frozen vs generated (2026-08-26 baseline)

- **`godot/maps/village/`** is the frozen baseline: no generator exists for it.
  Edit `village.tscn` / its `gen/` artifacts **directly**, through the normal
  art-review gates.
- Maps that still have live generators (`gen_hieda*.gd`, `gen_kourindou.gd`,
  `gen_trail.gd`, …): their `gen/*.res` and `*.tscn` are generated but
  tracked — regenerate and commit them together with the generator change;
