---
name: godot-master
description: "Godot 4.7 expert knowledge reference for this project (3D fan game, desktop-first). Trimmed 2026-08-26 to project-relevant domains: 3D lighting/materials/world-building, physics, navigation, procedural generation, animation (AnimationPlayer/AnimationTree), combat, shaders, particles, performance, debugging, GDScript, architecture patterns, UI, audio, save/load, desktop/web export. Use as an on-demand API/pattern lookup — load only the single reference file needed. Genre blueprints, 2D, mobile/console/VR, multiplayer, and the Builder/Agent Vision/Analyst/Auditor workflows were removed (parallel pipelines banned by AGENT_CONSTITUTION.md)."
---
...
| :--- | :--- | :--- | :--- |
| **Rapid Prototype** | Event-Driven Mono | **READ**: [Foundations](references/project-foundations.md) → [Autoloads](references/autoload-architecture.md). | Fast start, spaghetti risk |
| **Component Entities (player/NPC)** | Component-Driven | **READ**: [Composition](references/composition.md) → [States](references/state-machine-advanced.md). | Heavy setup, infinite scaling |
| **Large 3D World / Streaming** | Resource-Streaming | **READ**: [3D World Building](references/3d-world-building.md) → [Save/Load](references/save-load-systems.md) → [Performance](references/performance-optimization.md). | Complex I/O, float precision jitter past 10K units |
| **Melee / Character Combat** | Signal-Driven Combat | **READ**: [Combat](references/combat-system.md) → [Animation Tree](references/animation-tree-mastery.md) → [Physics 3D](references/physics-3d.md). | Animation-physics sync |
...
| **Cel-shading / Stylized Look** | Shader Pipeline | **READ**: [Shaders](references/shaders-basics.md) → [3D Lighting](references/3d-lighting.md) → [3D Materials](references/3d-materials.md). | Style consistency across assets |
...
**MANDATORY Chain — READ ALL**: [Composition](references/composition.md) → [State Machine](references/state-machine-advanced.md) → [Physics 3D](references/physics-3d.md) → [Animation Tree](references/animation-tree-mastery.md)
...
**MANDATORY Chain — READ ALL**: [Resource Patterns](references/resource-data-patterns.md) → [Combat](references/combat-system.md)
...
3. **Physics Stutter** → Simplify collisions to primitive shapes. Load [3D Physics](references/physics-3d.md). Check if `_process` is used instead of `_physics_process` for movement.
...
**READ as needed**: [Input Handling](references/input-handling.md), [Platform Desktop](references/platform-desktop.md), [Platform Web](references/platform-web.md)
...
**MANDATORY**: [Procedural Gen](references/procedural-generation.md) → [3D World Building](references/3d-world-building.md) → [Navigation](references/navigation-pathfinding.md)
...
## 📂 Part 9: Module Directory (trimmed to project-relevant domains, 2026-08-26)
...
> Removed 2026-08-26 (backup: `work_backups/godot-master-trimmed_20260826/`): all genre blueprints, 2D systems, mobile/console/VR platforms, adaptation guides, multiplayer/server, RPG/economy/inventory/quest/dialogue systems, game-loop and mechanic modules, seasonal theming, Monte Carlo balancer, and the Builder / Agent Vision / Analyst / Auditor personas (banned as parallel pipelines by `AGENT_CONSTITUTION.md`).
