# Godot Master — Expert Core (Frameworks, Workflows, Budgets, Patterns)

Moved out of SKILL.md 2026-08-27 to cut per-invocation token cost.
Load this file only for architecture-level questions (system design, signal
architecture, performance strategy, server APIs, Unity translation).
Note: reference links inside this file are relative to references/
...
- **Eyes → Verification**: after visual work, verify with **this project's capture pipeline and Human Art Review** (`.claude/rules/art-review.md`) — never verbal guesswork.
- **Shader precompile**: prevent runtime frame-hitches by instantiating hidden effects during loading to force GPU shader pipeline compilation.
- **Hitscan**: use `intersect_ray()` (direct space state) over `RayCast3D` nodes when performance matters.
...
| :--- | :--- | :--- | :--- |
| **Rapid Prototype** | Event-Driven Mono | **READ**: [Foundations](project-foundations.md) → [Autoloads](autoload-architecture.md). | Fast start, spaghetti risk |
| **Component Entities (player/NPC)** | Component-Driven | **READ**: [Composition](composition.md) → [States](state-machine-advanced.md). | Heavy setup, infinite scaling |
| **Large 3D World / Streaming** | Resource-Streaming | **READ**: [3D World Building](3d-world-building.md) → [Save/Load](save-load-systems.md) → [Performance](performance-optimization.md). | Complex I/O, float precision jitter past 10K units |
| **Melee / Character Combat** | Signal-Driven Combat | **READ**: [Combat](combat-system.md) → [Animation Tree](animation-tree-mastery.md) → [Physics 3D](physics-3d.md). | Animation-physics sync |
| **HDR / Cinematic Visuals** | Display Pipeline | **READ**: [3D Lighting](3d-lighting.md) → [Platform Desktop](platform-desktop.md) → [Shaders](shaders-basics.md). Enable viewport HDR in Project Settings. | Platform-specific tonemapping tuning |
| **Rectangular Area Lights** | AreaLight3D | **READ**: [3D Lighting](3d-lighting.md) → [3D Materials](3d-materials.md). Prefer AreaLight3D over emissive+GI hacks. | Forward+ renderer required for full quality |
| **Cel-shading / Stylized Look** | Shader Pipeline | **READ**: [Shaders](shaders-basics.md) → [3D Lighting](3d-lighting.md) → [3D Materials](3d-materials.md). | Style consistency across assets |
| **Addon / Asset Discovery** | Asset Store | **READ**: [Project Foundations](project-foundations.md) → [Export Builds](export-builds.md). Asset Store replaces Asset Library. | Beta store UI — verify licensing per addon |
| **Visual QA** | Project pipeline | Use this project's capture pipeline + Human Art Review (`.claude/rules/art-review.md`). Do not build a parallel capture/scoring framework. | — |
...
**MANDATORY — READ ENTIRE FILE**: [Foundations](project-foundations.md)
...
2. **READ**: [Signal Architecture](signal-architecture.md) — Create `GlobalSignalBus` autoload with < 15 events.
3. **READ**: [GDScript Mastery](gdscript-mastery.md) — Enable `untyped_declaration` warning in Project Settings → GDScript → Debugging.
4. Apply **[Project Templates](project-templates.md)** for base `.gitignore`, export presets, and input map.
...
**MANDATORY Chain — READ ALL**: [Composition](composition.md) → [State Machine](state-machine-advanced.md) → [Physics 3D](physics-3d.md) → [Animation Tree](animation-tree-mastery.md)
...
**MANDATORY Chain — READ ALL**: [Resource Patterns](resource-data-patterns.md) → [Combat](combat-system.md)
...
**MANDATORY**: [Autoload Architecture](autoload-architecture.md) → [Save/Load](save-load-systems.md) → [Scene Management](scene-management.md)
...
**MANDATORY**: [Debugging/Profiling](debugging-profiling.md) → [Performance Optimization](performance-optimization.md)
...
3. **Physics Stutter** → Simplify collisions to primitive shapes. Load [3D Physics](physics-3d.md). Check if `_process` is used instead of `_physics_process` for movement.
...
### Workflow 6: Platform / Input Notes (desktop-first, web optional)
**READ as needed**: [Input Handling](input-handling.md), [Platform Desktop](platform-desktop.md), [Platform Web](platform-web.md)

- Use an `InputManager` autoload that translates all input types into normalized actions. NEVER read `Input.is_key_pressed()` directly.
...
- **NEVER** pass unsanitized strings to `JavaScriptBridge.eval()` in web builds.

### Workflow 7: Procedural Generation
**MANDATORY**: [Procedural Gen](procedural-generation.md) → [3D World Building](3d-world-building.md) → [Navigation](navigation-pathfinding.md)
...
**MANDATORY Chain**: [UI Containers](ui-containers.md) → [UI Theming](ui-theming.md) → [Rich Text](ui-rich-text.md) → [Tweening](tweening.md)
...
6. **See it**: verify with this project's capture pipeline + Human Art Review (`.claude/rules/art-review.md`). Agents cannot QA UI from text alone.
...
> - **NEVER** declare UI “done” without an in-engine screenshot reviewed per `.claude/rules/art-review.md`.

### Workflow 10: Cinematic Lighting & VFX (Audit Verified)
**MANDATORY Chain**: [3D Lighting](3d-lighting.md) → [Particles](particles.md) → [3D Materials](3d-materials.md) → [Shaders](shaders-basics.md)
...
6. **See it**: verify lighting, exposure, and VFX in rendered screenshots via this project's capture pipeline (`.claude/rules/art-review.md`).
...
> - **NEVER** trust lighting “looks fine” from code alone — capture in-engine screenshots per `.claude/rules/art-review.md`.
...
| **Audio Buses** | < 8 simultaneous | < 32 simultaneous | Use [Audio Systems](audio-systems.md) bus routing |
...
- **[Component Registry](patterns/component_registry.md)**: Centralized dictionary-based component retrieval.
- **[Safe Signal Handler](patterns/safe_signal_handler.md)**: Preventing crashes on freed object references.
- **[Async Resource Loader](patterns/async_resource_loader.md)**: Threaded asset ingestion.
- **[State Machine Transition Guard](patterns/state_machine_transition_guard.md)**: Validating state changes.
- **[Thread-Safe Chunk Loader](patterns/thread_safe_chunk_loader.md)**: Low-level server-api construction.
- **[Vision Cone Detection](patterns/vision_cone_detection.md)**: Expert NPC vision with dot products and raycasts.
- **[Sound Propagation System](patterns/sound_propagation_system.md)**: Acoustic occlusion logic.
- **[Stealth Hiding Logic](patterns/stealth_hiding_logic.md)**: Global visibility and concealment management.
