...
- **Coyote time** — implement in the character controller's `_physics_process` (see [expert-input-extensions.md](input-handling-expert-input-extensions.md) for the timer pattern); pair with buffers from this skill.
...
- godot-characterbody-2d — Consumes buffered jump/dash and `get_vector` movement inside physics steps.
...
- godot-adapt-desktop-to-mobile — Virtual joysticks and touch cameras extend this skill’s multi-touch gesture patterns.
