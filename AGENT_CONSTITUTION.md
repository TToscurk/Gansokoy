...
以最短時間做出玩家**看得見、走得到、可展示**的幻想鄉品質提升。近期 showable slice 固定為：

**北門（North Gate）→ 本通（Main Street）→ 市場（Market）**
...
8. `sources/` 是唯讀參考，不得編輯、改名、移動或刪除；本地鏡像可能不完整，不得捏造缺失資料。
...
6. 進入 `INTEGRATION`，一次 placement、一次 Full Validation、一次北門→本通→市場前後對照。
