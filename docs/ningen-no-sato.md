...
| 水邊生物 | 鴨 9・鯉 14・鷺鷥 5 | — |
| 水生植物 | 睡蓮 220・荷 80 | 2 |
| 稗田邸院內植栽 | 生垣 29・紅葉 5・松 2・灌木 15 = 51 株 | 10 |
...
| 稗田邸 | (−78, −164) | 45.2 × 48.2 | 44.3 × 47.3 |
...
godot --headless --path godot --script tools/hieda_front_check.gd  # 稗田邸前庭規格
...
### `tools/hieda_front_check.gd` —— 稗田邸前庭定案規格
從存好的 `village.tscn` **量成品**（不量產生器參數），跟 `make_hieda.py` 的
明表逐項比：舊築地塀／藥醫門殘留、狛犬（含「嚴格對稱」）、石燈籠與松
（含「兩側疏密**不等**」）、棟門、格子塀六層、犬走り（含接觸陰影 albedo ≤0.15）、
參道、唐破風。存在的理由就是這一輪的病因：**兩套資產同名**，肉眼分不出來。
**驗不到**：構圖好不好看 —— 那還是只有截圖。
...
| | 村圖裡的稗田邸 | 獨立地圖用的稗田邸 |
|---|---|---|
| 在哪 | `village.tscn` 的地標之一，(−78, −164) | `assets/models/hieda_blockout.glb` + `data/hieda_garden.*.json` |
| 尺度 | 保留區 42.4 × 45.6 m | blockout 75.6 × 110.5 m、植栽擺位跨度 95.4 × 107.0 m |
| 內容 | **格子塀＋棟門＋狛犬＋石燈籠＋切石參道**＋二層入母屋（含唐破風玄関前廊）＋長廊／離れ＋石組庭池（東側）＋院內植栽 51 株 | 建築＋水池＋枯山水＋飛石＋燈籠＋木戶＋植栽 136 實例 |
| 狀態 | ✅ 完成 | ⏳ 還沒有地形／碰撞／出生點／meta／mapRegistry 條目 |

**線性差 ~2.3×**；村院塀內 38×41m 扣掉主屋／緣側／長廊／離れ／庭池之後，
真正能種東西的只有 **593 ㎡（38%）**，面積差 ~17×。所以後院**搬不進來**，
使用者定案：村院用同樣那 10 種模組**重排一套小的**，後院全套留給獨立地圖。

⚠ **兩套東西共用「稗田邸」這個名字，是踩過的坑**：Stage 0 的搬遷把
*舊 gen_village 的築地塀＋藥醫門* 當成定案前庭搬進了村圖，而使用者真正
逐輪驗收過的前庭在 `make_hieda.py`。2026-08-07 那輪已照定案規格重建村圖前庭
（格子塀／棟門／狛犬／石燈籠／切石參道 + 唐破風玄関前廊），對照工具是
`tools/hieda_front_check.gd`。細節見 `docs/hieda-estate-features.md`。

因此以下是**刻意保留的孤兒**，不要以為壞了就刪：
- `godot/maps/hieda/gen/mm_hieda_*.res`（10 個）
- `godot/assets/models/hieda_blockout.glb`、`hieda_main.glb`
- `godot/data/hieda_garden.markers.json` 裡 `target: null` 的木戶 portal
- `tools/gen_hieda.gd`（沒有人呼叫，等獨立地圖開工時直接 `emit()`）

已**脫離**孤兒狀態（村圖前庭在用）：`komainu_a.glb`、`stone_lantern.glb`。
村圖前庭是**照規格重建**的，不是從 blockout 切網格 —— blockout 是一份 join
過的 mesh（切不出來），而且切了就等於開第二個真相來源。
...
- **稗田邸獨立地圖**：資產都在，缺地形／碰撞／出生點／portal 目的地／meta／registry 條目
